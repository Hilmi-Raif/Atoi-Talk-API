...
func TestStorageAdapterDeleteContextUsesCallerContext(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Fatal("expected cancelled request not to reach S3")
	}))
	defer server.Close()
	...
	if err := adapter.DeleteContext(ctx, "file.txt", false); err == nil {
...
func TestStorageAdapterPresignedRequestsUsePresignEndpoint(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	defer server.Close()
	...
	adapter := NewStorageAdapter(&config.AppConfig{
		S3BucketPublic:    "public",
		S3BucketPrivate:   "private",
		S3Region:          "us-east-1",
		S3Endpoint:        server.URL,
		S3PresignEndpoint: "http://custom-presign.local:9090",
	}, client, server.Client())

	getURL, err := adapter.GetPresignedURL("file.txt", time.Minute)
	if err != nil || !strings.HasPrefix(getURL, "http://custom-presign.local:9090/private/file.txt") {
		t.Fatalf("unexpected presigned GET URL: %q, %v", getURL, err)
	}

	putURL, _, err := adapter.GetPresignedPutURL("file.txt", "text/plain", 12, false, time.Minute)
	if err != nil || !strings.HasPrefix(putURL, "http://custom-presign.local:9090/private/file.txt") {
