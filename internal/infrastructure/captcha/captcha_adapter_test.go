...
func TestCaptchaAdapterVerifyRejectsOversizedResponse(t *testing.T) {
	...
	adapter := NewCaptchaAdapter(&config.AppConfig{TurnstileSecretKey: "secret", CaptchaEnabled: true}, httpClient)
...
func TestCaptchaAdapterVerifyRejectsProviderStatus(t *testing.T) {
	adapter := &CaptchaAdapter{
		cfg: &config.AppConfig{TurnstileSecretKey: "secret", CaptchaEnabled: true},
...
func TestCaptchaAdapterVerifyBypassesWhenDisabled(t *testing.T) {
	adapter := &CaptchaAdapter{
		cfg: &config.AppConfig{CaptchaEnabled: false},
	}
	if err := adapter.Verify("any-token", "127.0.0.1"); err != nil {
		t.Fatalf("expected nil error when captcha is disabled, got %v", err)
	}
}
