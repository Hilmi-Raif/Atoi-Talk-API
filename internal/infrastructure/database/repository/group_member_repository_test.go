...
func TestGroupMemberRepositoryCountActiveMembersCachesResult(t *testing.T) {
	client := enttest.Open(t, dialect.SQLite, "file:group-member-repository-cache?mode=memory&cache=shared&_fk=1")
	defer func() { _ = client.Close() }()
	ctx := context.Background()
	...
	group := client.GroupChat.Create().
		SetChatID(parentChat.ID).
		SetName("cache_group").
		SetInviteCode("invite_cache").
		SetIsPublic(true).
		SaveX(ctx)
	client.GroupMember.Create().SetGroupChatID(group.ID).SetUserID(u1.ID).SaveX(ctx)

	repo := NewGroupMemberRepository(client)
	counts1, err := repo.CountActiveMembersByGroupIDs(ctx, group.ID)
	if err != nil {
		t.Fatalf("first count failed: %v", err)
	}
	if counts1[group.ID] != 1 {
		t.Fatalf("expected 1, got %v", counts1[group.ID])
	}

	client.GroupMember.Create().SetGroupChatID(group.ID).SetUserID(u2.ID).SaveX(ctx)

	counts2, err := repo.CountActiveMembersByGroupIDs(ctx, group.ID)
	if err != nil {
		t.Fatalf("second count failed: %v", err)
	}
	if counts2[group.ID] != 1 {
		t.Fatalf("expected cached count 1, got %v", counts2[group.ID])
	}

	repo.InvalidateGroupMemberCount(group.ID)
	counts3, err := repo.CountActiveMembersByGroupIDs(ctx, group.ID)
	if err != nil {
		t.Fatalf("third count failed: %v", err)
	}
	if counts3[group.ID] != 2 {
		t.Fatalf("expected refreshed count 2, got %v", counts3[group.ID])
	}
}
