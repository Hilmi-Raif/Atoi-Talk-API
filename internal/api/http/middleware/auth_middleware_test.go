...
func TestVerifyTokenEmitsAuthenticationPhaseSpans(t *testing.T) {
	...
	handler := (&AuthMiddleware{authService: auth, sessionRepo: session}).VerifyToken(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusNoContent)
	}))
	request := httptest.NewRequest(http.MethodGet, "/", nil)
	request.Header.Set("Authorization", "Bearer token")
	...
	require.True(t, names["auth.blacklist"])
	require.True(t, names["auth.verify_user"])
}
