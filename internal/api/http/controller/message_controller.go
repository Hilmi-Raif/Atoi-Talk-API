...
func (c *MessageController) EditMessage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		response.WriteError(w, helper.NewBadRequestError("Invalid Message ID"))
		return
	}
...
func (c *MessageController) GetMessages(w http.ResponseWriter, r *http.Request) {
	...
	if aroundMessageIDStr != "" {
		...
		if err != nil {
			response.WriteError(w, helper.NewBadRequestError("Invalid around_message_id format"))
			return
		}
		aroundMessageID = &id
	}
	...
	response.WriteSuccessWithPaginationBidirectional(w, messages, nextCursor, hasNext, prevCursor, hasPrev)
}
