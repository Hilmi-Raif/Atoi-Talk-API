...
func (c *UserController) SearchUsers(w http.ResponseWriter, r *http.Request) {
	...
	if includeChatIDStr != "" {
		...
		if err != nil {
			response.WriteError(w, helper.NewBadRequestError("Invalid include_chat_id"))
			return
		}
		includeChatID = b
	}
	...
	if excludeChatIDStr != "" {
		if id, err := uuid.Parse(excludeChatIDStr); err == nil {
			...
			response.WriteError(w, helper.NewBadRequestError("Invalid exclude_chat_id"))
			return
		}
	}
	...
	response.WriteSuccessWithPagination(w, users, nextCursor, hasNext)
}
