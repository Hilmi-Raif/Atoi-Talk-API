...
import (
	...
	servicemocks "AtoiTalkAPI/internal/api/application/mocks"
...
func TestAuthServiceVerifyUserCachesUserLookup(t *testing.T) {
	client := newAuthTestClient(t)
	defer func() { _ = client.Close() }()
	service, _, _, session, _ := newMockAuthService(t, client)
	ctx := context.Background()
	u := client.User.Create().SetUsername("cached_user").SetRole(user.RoleUser).SaveX(ctx)
	token, err := helper.GenerateJWT("test-secret", 3600, u.ID)
	require.NoError(t, err)
	session.EXPECT().IsUserRevoked(mock.Anything, u.ID, mock.AnythingOfType("int64")).Return(false, nil).Times(3)

	res1, err := service.VerifyUser(ctx, token)
	require.NoError(t, err)
	require.Equal(t, u.ID, res1.ID)

	client.User.DeleteOneID(u.ID).ExecX(ctx)

	res2, err := service.VerifyUser(ctx, token)
	require.NoError(t, err)
	require.Equal(t, u.ID, res2.ID)

	service.InvalidateUserCache(u.ID)
	res3, err := service.VerifyUser(ctx, token)
	require.Error(t, err)
	require.Nil(t, res3)
}

func TestAuthServiceVerifyUserEmitsAuthenticationDependencySpans(t *testing.T) {
	...
	client := newAuthTestClient(t)
	userID := uuid.New()
	client.User.Create().SetID(userID).SetFullName("Auth User").SetEmail("auth@example.com").SetRole(user.RoleUser).SaveX(context.Background())
	service, _, _, session, _ := newMockAuthService(t, client)
	session.EXPECT().IsUserRevoked(mock.Anything, userID, mock.Anything).Return(false, nil)
	token, err := helper.GenerateJWT("test-secret", 3600, userID)
	require.NoError(t, err)

	_, err = service.VerifyUser(context.Background(), token)
	require.NoError(t, err)
	...
	require.True(t, names["auth.jwt_parse"])
	require.True(t, names["auth.revoke_lookup"])
	require.True(t, names["auth.user_lookup"])
}
