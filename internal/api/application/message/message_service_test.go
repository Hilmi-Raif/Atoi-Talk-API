...
import (
	...
	adaptermocks "AtoiTalkAPI/internal/infrastructure/mocks"
...
func TestRunPaginationAvailabilityChecksSequentially(t *testing.T) {
	...
	go func() {
		_, _, err := runPaginationAvailabilityChecks(context.Background(), check, check)
		done <- err
	}()
	...
	case <-time.After(time.Second):
		t.Fatal("expected first pagination check to start")
	}
	select {
	case <-secondStarted:
		t.Fatal("second pagination check started before the first completed")
	...
	case err := <-done:
		if err != nil {
			t.Fatalf("run pagination checks: %v", err)
		}
	case <-time.After(time.Second):
		t.Fatal("sequential pagination checks did not finish")
	}
	if maxActive.Load() != 1 {
		t.Fatalf("expected pagination checks to use one connection at a time, max active=%d", maxActive.Load())
	}
}
...
func TestMessageServiceGetMessagesMapsPrivateMessages(t *testing.T) {
	...
	messageRepo.EXPECT().GetMessages(mock.Anything, chatEntity.ID, (*time.Time)(nil), uuid.Nil, 10, "older").Return([]*ent.Message{loadedMessage}, nil)
...
func TestMessageServiceGetMessagesEmitsPhaseSpans(t *testing.T) {
	service, client, messageRepo := newMessageServiceTest(t)
	defer client.Close()
	ctx, current, _, chatEntity, msg := createPrivateMessageServiceFixture(t, client)
	loadedMessage := loadMessageServiceFixture(ctx, client, msg.ID)
	...
	_, _, _, _, _, err := service.GetMessages(ctx, current.ID, model.GetMessagesRequest{
		ChatID:    chatEntity.ID,
		Limit:     10,
		Direction: "older",
	})
	...
	require.True(t, names["service.message.database.chat_access"])
	require.True(t, names["service.message.repository.fetch"])
	require.True(t, names["service.message.database.pagination"])
}

func TestMessageServiceGetMessagesDoesNotLoadGroupChatForPrivateChat(t *testing.T) {
	var groupChatQueryCount atomic.Int32
	client := enttest.Open(t, dialect.SQLite, "file:message-service-private-access-query-count?mode=memory&cache=shared&_fk=1", enttest.WithOptions(
		ent.Debug(),
		ent.Log(func(values ...any) {
			statement := strings.ToLower(fmt.Sprint(values...))
			if strings.Contains(statement, "from") && strings.Contains(statement, "group") {
				groupChatQueryCount.Add(1)
			}
		}),
	))
	defer client.Close()
	...
	current := client.User.Create().SetUsername("private-access-current").SaveX(ctx)
	other := client.User.Create().SetUsername("private-access-other").SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypePrivate).SaveX(ctx)
	client.PrivateChat.Create().SetChat(chatEntity).SetUser1ID(current.ID).SetUser2ID(other.ID).SaveX(ctx)
	msg := client.Message.Create().SetChatID(chatEntity.ID).SetSenderID(current.ID).SetType(message.TypeRegular).SetContent("private access").SaveX(ctx)
	loadedMessage := loadMessageServiceFixture(ctx, client, msg.ID)
	messageRepo.EXPECT().GetMessages(mock.Anything, chatEntity.ID, (*time.Time)(nil), uuid.Nil, 10, "older").Return([]*ent.Message{loadedMessage}, nil)

	groupChatQueryCount.Store(0)
	_, _, _, _, _, err := service.GetMessages(ctx, current.ID, model.GetMessagesRequest{
		ChatID:    chatEntity.ID,
		Limit:     10,
		Direction: "older",
	...
	require.NoError(t, err)
	require.Equal(t, int32(0), groupChatQueryCount.Load())
}

func TestMessageServiceGetMessagesUsesOneAvailabilityQueryWhenPageHasExtraMessage(t *testing.T) {
	var queryCount atomic.Int32
	client := enttest.Open(t, dialect.SQLite, "file:message-service-pagination-query-count?mode=memory&cache=shared&_fk=1", enttest.WithOptions(
		ent.Debug(),
		ent.Log(func(values ...any) {
			if len(values) > 0 && strings.Contains(fmt.Sprint(values...), "SELECT `messages`.`id` FROM `messages`") {
				queryCount.Add(1)
			}
		}),
	))
	...
	current := client.User.Create().SetUsername("pagination-current").SaveX(ctx)
	other := client.User.Create().SetUsername("pagination-other").SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypePrivate).SaveX(ctx)
	client.PrivateChat.Create().SetChatID(chatEntity.ID).SetUser1ID(current.ID).SetUser2ID(other.ID).SaveX(ctx)
	first := client.Message.Create().SetChatID(chatEntity.ID).SetSenderID(current.ID).SetType(message.TypeRegular).SetContent("first").SaveX(ctx)
	second := client.Message.Create().SetChatID(chatEntity.ID).SetSenderID(current.ID).SetType(message.TypeRegular).SetContent("second").SaveX(ctx)
	third := client.Message.Create().SetChatID(chatEntity.ID).SetSenderID(current.ID).SetType(message.TypeRegular).SetContent("third").SaveX(ctx)
	firstLoaded := loadMessageServiceFixture(ctx, client, first.ID)
	secondLoaded := loadMessageServiceFixture(ctx, client, second.ID)
	thirdLoaded := loadMessageServiceFixture(ctx, client, third.ID)
	messageRepo.EXPECT().GetMessages(mock.Anything, chatEntity.ID, (*time.Time)(nil), uuid.Nil, 2, "older").Return([]*ent.Message{thirdLoaded, secondLoaded, firstLoaded}, nil)

	queryCount.Store(0)
	_, _, hasNext, _, hasPrev, err := service.GetMessages(ctx, current.ID, model.GetMessagesRequest{
		ChatID:    chatEntity.ID,
		Limit:     2,
		Direction: "older",
	})
...
func TestMessageServiceGetMessagesSkipsAvailabilityQueryForNewerPageWithExtraMessage(t *testing.T) {
	var queryCount atomic.Int32
	client := enttest.Open(t, dialect.SQLite, "file:message-service-newer-pagination-query-count?mode=memory&cache=shared&_fk=1", enttest.WithOptions(
		ent.Debug(),
		ent.Log(func(values ...any) {
			if len(values) > 0 && strings.Contains(fmt.Sprint(values...), "SELECT `messages`.`id` FROM `messages`") {
				queryCount.Add(1)
	...
	current := client.User.Create().SetUsername("newer-pagination-current").SaveX(ctx)
	other := client.User.Create().SetUsername("newer-pagination-other").SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypePrivate).SaveX(ctx)
	client.PrivateChat.Create().SetChatID(chatEntity.ID).SetUser1ID(current.ID).SetUser2ID(other.ID).SaveX(ctx)
	first := client.Message.Create().SetChatID(chatEntity.ID).SetSenderID(current.ID).SetType(message.TypeRegular).SetContent("first").SaveX(ctx)
	second := client.Message.Create().SetChatID(chatEntity.ID).SetSenderID(current.ID).SetType(message.TypeRegular).SetContent("second").SaveX(ctx)
	...
	secondLoaded := loadMessageServiceFixture(ctx, client, second.ID)
	thirdLoaded := loadMessageServiceFixture(ctx, client, third.ID)
	messageRepo.EXPECT().GetMessages(mock.Anything, chatEntity.ID, (*time.Time)(nil), uuid.Nil, 2, "newer").Return([]*ent.Message{firstLoaded, secondLoaded, thirdLoaded}, nil)
	...
	require.Equal(t, int32(0), queryCount.Load())
}

func TestMessageServiceSendMessageEmitsTransactionSpan(t *testing.T) {
	...
	require.True(t, names["service.message.transaction"])
	require.True(t, names["service.message.response.fetch"])
}

func TestMessageServiceSendMessageSkipsUnusedResponseEdges(t *testing.T) {
	var mediaQueries atomic.Int32
	var messageQueries atomic.Int32
	client := enttest.Open(t, dialect.SQLite, "file:message-service-response-edge-query-count?mode=memory&cache=shared&_fk=1", enttest.WithOptions(
		ent.Debug(),
		ent.Log(func(values ...any) {
			statement := fmt.Sprint(values...)
			if strings.Contains(statement, "FROM `media`") {
				mediaQueries.Add(1)
			}
			if strings.Contains(statement, "FROM `messages`") {
	...
	service := NewMessageService(
		client,
		repositorymocks.NewMockMessageReader(t),
		&config.AppConfig{},
		config.NewValidator(),
		adaptermocks.NewMockURLGenerator(t),
		nil,
	)
	sender := client.User.Create().SetUsername("response-edge-sender").SetFullName("Sender").SaveX(ctx)
	recipient := client.User.Create().SetUsername("response-edge-recipient").SetFullName("Recipient").SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypePrivate).SaveX(ctx)
	client.PrivateChat.Create().SetChatID(chatEntity.ID).SetUser1ID(sender.ID).SetUser2ID(recipient.ID).SaveX(ctx)
	mediaQueries.Store(0)
	messageQueries.Store(0)

	_, err := service.SendMessage(ctx, sender.ID, model.SendMessageRequest{
		ChatID:  chatEntity.ID,
		Content: "response edge query",
	})

	require.NoError(t, err)
	require.Equal(t, int32(0), mediaQueries.Load())
	require.Equal(t, int32(0), messageQueries.Load())
}

func TestMessageServiceSendMessageUsesCreatedMessageForSimpleResponse(t *testing.T) {
	var responseMessageQueries atomic.Int32
	client := enttest.Open(t, dialect.SQLite, "file:message-service-simple-response-query-count?mode=memory&cache=shared&_fk=1", enttest.WithOptions(
		ent.Debug(),
		ent.Log(func(values ...any) {
			statement := fmt.Sprint(values...)
			if strings.Contains(statement, "SELECT") && strings.Contains(statement, "FROM `messages`") {
				responseMessageQueries.Add(1)
			}
		}),
	))
	defer client.Close()
	...
	sender := client.User.Create().SetUsername("simple-response-sender").SetFullName("Sender").SaveX(ctx)
	recipient := client.User.Create().SetUsername("simple-response-recipient").SetFullName("Recipient").SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypePrivate).SaveX(ctx)
	client.PrivateChat.Create().SetChatID(chatEntity.ID).SetUser1ID(sender.ID).SetUser2ID(recipient.ID).SaveX(ctx)
	responseMessageQueries.Store(0)
	...
	require.Equal(t, "simple response", result.Content)
	require.Equal(t, "Sender", result.SenderName)
	require.Equal(t, int32(0), responseMessageQueries.Load())
}
...
func TestMessageServiceGetMessagesUsesAroundRepositoryPath(t *testing.T) {
	...
	ctx, current, _, chatEntity, msg := createPrivateMessageServiceFixture(t, client)
	loadedMessage := loadMessageServiceFixture(ctx, client, msg.ID)
	messageRepo.EXPECT().GetMessagesAround(mock.Anything, chatEntity.ID, (*time.Time)(nil), msg.ID, 10).
...
func TestMessageServiceGetMessagesMapsNotFoundRepositoryError(t *testing.T) {
	...
	defer client.Close()
	ctx, current, _, chatEntity, _ := createPrivateMessageServiceFixture(t, client)
	messageRepo.EXPECT().GetMessages(mock.Anything, chatEntity.ID, (*time.Time)(nil), uuid.Nil, 10, "older").
...
func TestMessageServiceDeleteMessageAllowsGroupAdminAndPersistsOutbox(t *testing.T) {
	service, client, _ := newMessageServiceTest(t)
	defer client.Close()
	ctx := context.Background()
	...
	outbox, err := client.MessageOutbox.Query().Where(
		...
		messageoutbox.MessageID(msg.ID),
		messageoutbox.EventTypeEQ(string(websocket.EventMessageDelete)),
	).Only(ctx)
	require.NoError(t, err)
	require.Equal(t, msg.ID, outbox.MessageID)
}
...
func TestMessageServiceSendMessageCreatesGroupMessageAndUpdatesUnreadCounts(t *testing.T) {
	...
	require.Equal(t, 0, recipientMember.UnreadCount)
	require.Equal(t, 0, senderMember.UnreadCount)
}

func TestMessageServiceSendMessageDoesNotLoadPrivateChatForGroupMessage(t *testing.T) {
	var privateChatQueries atomic.Int32
	client := enttest.Open(t, dialect.SQLite, "file:message-service-group-chat-query-count?mode=memory&cache=shared&_fk=1", enttest.WithOptions(
		ent.Debug(),
		ent.Log(func(values ...any) {
			statement := fmt.Sprint(values...)
			if strings.Contains(statement, "FROM `private_chats`") {
				privateChatQueries.Add(1)
			}
		}),
	))
	defer client.Close()
	...
	sender := client.User.Create().SetUsername("group-query-sender").SaveX(ctx)
	recipient := client.User.Create().SetUsername("group-query-recipient").SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypeGroup).SaveX(ctx)
	group := client.GroupChat.Create().SetChatID(chatEntity.ID).SetName("group-query").SetInviteCode("group-query-invite").SaveX(ctx)
	client.GroupMember.Create().SetGroupChatID(group.ID).SetUserID(sender.ID).SetRole(groupmember.RoleOwner).SaveX(ctx)
	client.GroupMember.Create().SetGroupChatID(group.ID).SetUserID(recipient.ID).SetRole(groupmember.RoleMember).SaveX(ctx)
	privateChatQueries.Store(0)
	...
	require.NoError(t, err)
	require.Equal(t, int32(0), privateChatQueries.Load())
}

func TestMessageServiceSendMessageUpdatesGroupMembersInOneStatement(t *testing.T) {
	var groupMemberUpdates atomic.Int32
	client := enttest.Open(t, dialect.SQLite, "file:message-service-group-member-update-count?mode=memory&cache=shared&_fk=1", enttest.WithOptions(
		ent.Debug(),
		ent.Log(func(values ...any) {
			statement := fmt.Sprint(values...)
			if strings.Contains(statement, "UPDATE") && strings.Contains(statement, "group_members") {
				groupMemberUpdates.Add(1)
			}
		}),
	))
	defer client.Close()
	...
	sender := client.User.Create().SetUsername("group-update-sender").SaveX(ctx)
	recipient := client.User.Create().SetUsername("group-update-recipient").SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypeGroup).SaveX(ctx)
	group := client.GroupChat.Create().SetChatID(chatEntity.ID).SetName("group-update").SetInviteCode("group-update-invite").SaveX(ctx)
	client.GroupMember.Create().SetGroupChatID(group.ID).SetUserID(sender.ID).SetRole(groupmember.RoleOwner).SaveX(ctx)
	client.GroupMember.Create().SetGroupChatID(group.ID).SetUserID(recipient.ID).SetRole(groupmember.RoleMember).SaveX(ctx)
	groupMemberUpdates.Store(0)
	...
	require.NoError(t, err)
	require.Equal(t, int32(0), groupMemberUpdates.Load())
	recipientMember := client.GroupMember.Query().Where(groupmember.GroupChatID(group.ID), groupmember.UserID(recipient.ID)).OnlyX(ctx)
	senderMember := client.GroupMember.Query().Where(groupmember.GroupChatID(group.ID), groupmember.UserID(sender.ID)).OnlyX(ctx)
	require.Equal(t, 0, recipientMember.UnreadCount)
	require.Equal(t, 0, senderMember.UnreadCount)
...
func TestMessageServiceSendMessageUpdatesChatBeforeGroupMembers(t *testing.T) {
	var updateOrder []string
	client := enttest.Open(t, dialect.SQLite, "file:message-service-update-order?mode=memory&cache=shared&_fk=1", enttest.WithOptions(
		ent.Debug(),
		ent.Log(func(values ...any) {
			statement := fmt.Sprint(values...)
			if strings.Contains(statement, "UPDATE `chats`") {
				updateOrder = append(updateOrder, "chats")
			}
			if strings.Contains(statement, "UPDATE `group_members`") {
				updateOrder = append(updateOrder, "group_members")
			}
		}),
	))
	defer client.Close()
	...
	sender := client.User.Create().SetUsername("group-order-sender").SaveX(ctx)
	recipient := client.User.Create().SetUsername("group-order-recipient").SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypeGroup).SaveX(ctx)
	group := client.GroupChat.Create().SetChatID(chatEntity.ID).SetName("group-order").SetInviteCode("group-order-invite").SaveX(ctx)
	client.GroupMember.Create().SetGroupChatID(group.ID).SetUserID(sender.ID).SetRole(groupmember.RoleOwner).SaveX(ctx)
	client.GroupMember.Create().SetGroupChatID(group.ID).SetUserID(recipient.ID).SetRole(groupmember.RoleMember).SaveX(ctx)
	updateOrder = nil
...
func TestMessageServiceSendMessageAndEditMessageBroadcasts(t *testing.T) {
	...
	_, err = client.MessageOutbox.Query().Where(
		messageoutbox.ChatID(chatEntity.ID),
		messageoutbox.MessageID(msg.ID),
...
func TestMessageServiceSendMessageFromUser2LeavesUnreadProjectionToWorker(t *testing.T) {
	service, client, _ := newMessageServiceTest(t)
	defer client.Close()
	ctx := context.Background()
...
func TestMessageServiceEditMessageInGroupChatAndPersistsOutbox(t *testing.T) {
	service, client, _ := newMessageServiceTest(t)
	defer client.Close()
	ctx := context.Background()
...
func TestMessageServiceSendMessageInGroupChatWithReplyAndBroadcasts(t *testing.T) {
	...
	require.Equal(t, 0, u2Member.UnreadCount)
...
func TestMessageServiceSendMessageAndEditMessageGroupAttachmentsWithBroadcast(t *testing.T) {
	...
	_, err = client.MessageOutbox.Query().Where(
		messageoutbox.ChatID(chatEntity.ID),
		messageoutbox.MessageID(sentMsg.ID),
		messageoutbox.EventTypeEQ(string(websocket.EventMessageUpdate)),
	).Only(ctx)
	require.NoError(t, err)
}
...
func TestMessageServiceSendMessageAndEditMessageWithAvatarsAndBroadcasts(t *testing.T) {
	...
	_, err = client.MessageOutbox.Query().Where(
		messageoutbox.ChatID(chatEntity.ID),
		messageoutbox.MessageID(sent.ID),
		messageoutbox.EventTypeEQ(string(websocket.EventMessageUpdate)),
	).Only(ctx)
	require.NoError(t, err)
}
...
func TestMessageServiceSendMessageUpdatesPrivateChatCountersForSender(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, 5, updatedPC.User1UnreadCount)
	require.Equal(t, 0, updatedPC.User2UnreadCount)
	require.Nil(t, updatedPC.User1LastReadAt)
	...
	require.Equal(t, 5, updatedPC2.User1UnreadCount)
	require.Nil(t, updatedPC2.User2LastReadAt)
}
