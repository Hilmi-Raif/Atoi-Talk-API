...
import (
	...
	"AtoiTalkAPI/internal/messaging/outbox"
	"context"
	"fmt"
	"log/slog"
	"strings"
...
func (s *GroupChatService) getGroupLastMessageResponse(ctx context.Context, chatID uuid.UUID) *model.MessageResponse {
	...
	return mapper.ToMessageResponse(lastMsg, s.storageAdapter, nil, "")
}

func NewGroupChatService(client *ent.Client, groupMemberRepo repository.GroupMemberReader, groupChatRepo repository.GroupChatReader, cfg *config.AppConfig, validator *validator.Validate, wsHub events.Publisher, storageAdapter objectstorage.URLGenerator, redisAdapter redisinfra.Cache) *GroupChatService {
...
func (s *GroupChatService) CreateGroupChat(ctx context.Context, creatorID uuid.UUID, req model.CreateGroupChatRequest) (*model.ChatListResponse, error) {
	...
	if err := outbox.PersistMessageOutbox(ctx, tx, systemMsg, events.EventMessageNew, creatorID); err != nil {
		slog.Error("Failed to persist system message outbox event", "error", err)
		return nil, helper.NewInternalServerError("")
	}
	...
	if err == nil {
		lastMsgResp = mapper.ToMessageResponse(fullMsg, s.storageAdapter, nil, string(groupmember.RoleOwner))
	}
...
func (s *GroupChatService) UpdateGroupChat(ctx context.Context, requestorID uuid.UUID, groupID uuid.UUID, req model.UpdateGroupChatRequest, isAdmin bool) (*model.ChatListResponse, error) {
	...
	if len(systemMessages) > 0 {
		...
		if err := outbox.PersistMessageOutboxes(ctx, tx, msgs, events.EventMessageNew, requestorID); err != nil {
			slog.Error("Failed to persist system message outbox events", "error", err)
			return nil, helper.NewInternalServerError("")
		}
	...
	if s.wsHub != nil && len(createdSystemMessages) > 0 {
		go func() {
			...
			lastMsgResponse := mapper.ToMessageResponse(fullMsgByID[createdSystemMessages[len(createdSystemMessages)-1].ID], s.storageAdapter, nil, string(requestorRole))
			...
			for _, m := range members {
				...
				s.wsHub.BroadcastToUser(m.UserID, events.Event{
					Type:    events.EventChatUpdate,
					Payload: payload,
					Meta: &events.EventMeta{
						Timestamp: time.Now().UTC().UnixMilli(),
						ChatID:    gc.ChatID,
