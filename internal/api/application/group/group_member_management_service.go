...
func (s *GroupChatService) AddMember(ctx context.Context, requestorID uuid.UUID, groupID uuid.UUID, req model.AddGroupMemberRequest) ([]*model.MessageResponse, error) {
	...
	if len(req.UserIDs) > constant.MaxGroupMembers {
		return nil, helper.NewBadRequestError("")
	}
	if err := helper.EnsureUniqueUUIDs(req.UserIDs); err != nil {
		return nil, helper.NewBadRequestError("")
	}
	...
	if err == nil {
		for i, fullMsg := range fullMsgs {
			msgResponse := mapper.ToMessageResponse(fullMsg, s.storageAdapter, nil, string(requestorMember.Role))
	...
	if s.wsHub != nil {
		go func() {
			if len(responses) > 0 {
				...
				for _, u := range newMembers {
					chatPayload := s.buildGroupChatListResponse(gc, &memberRole, lastMsgResponse, memberCount)
					chatPayload.UnreadCount = 1
					chatPayload.MemberCount = memberCount

					s.wsHub.BroadcastToUser(u.ID, events.Event{
...
func (s *GroupChatService) LeaveGroup(ctx context.Context, userID uuid.UUID, groupID uuid.UUID) (*model.MessageResponse, error) {
	...
	if err := outbox.PersistMessageOutbox(ctx, tx, systemMsg, events.EventMessageNew, userID); err != nil {
	...
	if err == nil {
		msgResponse = mapper.ToMessageResponse(fullMsg, s.storageAdapter, nil, string(groupmember.RoleMember))
		msgResponse.MemberCount = &memberCount
	}
...
func (s *GroupChatService) KickMember(ctx context.Context, requestorID uuid.UUID, groupID uuid.UUID, targetUserID uuid.UUID) (*model.MessageResponse, error) {
	...
	if err := outbox.PersistMessageOutbox(ctx, tx, systemMsg, events.EventMessageNew, requestorID); err != nil {
		slog.Error("Failed to persist system message outbox event", "error", err)
		return nil, helper.NewInternalServerError("")
	}
	...
	if s.wsHub != nil && msgResponse != nil {
		go func() {
			s.wsHub.BroadcastToUser(targetUserID, events.Event{
