...
func TestGroupChatServiceJoinPublicGroupSuccess(t *testing.T) {
	...
	_, err = client.MessageOutbox.Query().Where(
		messageoutbox.ChatID(parent.ID),
		messageoutbox.MessageID(resp.LastMessage.ID),
		messageoutbox.EventTypeEQ(string(websocket.EventMessageNew)),
	).Only(ctx)
	require.NoError(t, err)
}
...
func TestGroupChatServiceJoinPublicGroupBroadcasts(t *testing.T) {
	...
	require.Equal(t, 1, client.MessageOutbox.Query().Where(
		messageoutbox.ChatID(parent.ID),
		messageoutbox.EventTypeEQ(string(websocket.EventMessageNew)),
	).CountX(ctx))
