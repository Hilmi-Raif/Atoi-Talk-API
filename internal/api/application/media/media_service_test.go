...
func TestMediaServiceCompleteUploadMarksMediaCompleted(t *testing.T) {
	...
	record := client.Media.Create().
		...
		SaveX(ctx)
	storage.EXPECT().Head("avatars/avatar.png", true).Return(&objectstorage.StorageObjectInfo{Size: 100, ContentType: "image/png"}, nil)
...
func TestMediaServiceCompleteUploadRejectsOwnershipAndSizeMismatch(t *testing.T) {
	...
	storage.EXPECT().Head("file.bin", false).Return(&objectstorage.StorageObjectInfo{Size: 99, ContentType: "application/octet-stream"}, nil)
	result, err = service.CompleteUpload(ctx, owner.ID, record.ID)
	require.Nil(t, result)
	require.Error(t, err)
}
...
func TestMediaServiceCompleteUploadRejectsInvalidStateAndContentType(t *testing.T) {
	...
	mismatch := client.Media.Create().
		...
		SetUploaderID(owner.ID).
		SaveX(ctx)
	storage.EXPECT().Head("mismatch.png", true).Return(&objectstorage.StorageObjectInfo{Size: 10, ContentType: "image/jpeg"}, nil)
...
func TestMediaServiceCompleteUploadAcceptsCompatibleAndEmptyContentType(t *testing.T) {
	...
	zipRecord := client.Media.Create().
		...
		SetUploaderID(owner.ID).
		SaveX(ctx)
	storage.EXPECT().Head("archive.zip", false).Return(&objectstorage.StorageObjectInfo{Size: 10, ContentType: "application/octet-stream"}, nil)
	...
	emptyTypeRecord := client.Media.Create().
		...
		SetUploaderID(owner.ID).
		SaveX(ctx)
	storage.EXPECT().Head("unknown.bin", false).Return(&objectstorage.StorageObjectInfo{Size: 10, ContentType: ""}, nil)
...
func TestMediaServiceCompleteUploadReturnsPrivateURLAndHandlesURLFailure(t *testing.T) {
	...
	record := client.Media.Create().
		...
		SetUploaderID(owner.ID).
		SaveX(ctx)
	storage.EXPECT().Head("file.bin", false).Return(&objectstorage.StorageObjectInfo{Size: 10, ContentType: "application/octet-stream"}, nil)
...
func TestMediaServiceCompleteUploadPublicAvatarReturnsPublicURL(t *testing.T) {
	...
	storage.EXPECT().Head("avatars/user.png", true).Return(&objectstorage.StorageObjectInfo{
		Size:        120,
