...
func TestUserServiceSearchUsersPropagatesServiceSpan(t *testing.T) {
	reader := repositorymocks.NewMockUserReader(t)
	storage := adaptermocks.NewMockPublicURLGenerator(t)
	presence := adaptermocks.NewMockRedisPresence(t)
	currentUserID := uuid.New()
	...
	reader.EXPECT().SearchUsers(mock.Anything, currentUserID, "", "", 10, (*uuid.UUID)(nil)).Run(func(ctx context.Context, _ uuid.UUID, _ string, _ string, _ int, _ *uuid.UUID) {
	...
	service := NewUserService(nil, reader, &config.AppConfig{}, config.NewValidator(), storage, nil, presence)
	_, _, _, err := service.SearchUsers(context.Background(), currentUserID, model.SearchUserRequest{Limit: 10})

	require.NoError(t, err)
	require.True(t, spanSeen)
}

func TestUserServiceSearchUsersEmitsPhaseSpans(t *testing.T) {
	service, client, _, _ := newEntUserService(t)
	defer client.Close()
	ctx := context.Background()
	current := client.User.Create().SetUsername("current").SaveX(ctx)
	...
	service.userRepo = repositorymocks.NewMockUserReader(t)
	service.userRepo.(*repositorymocks.MockUserReader).EXPECT().SearchUsers(mock.Anything, current.ID, "target", "", 10, (*uuid.UUID)(nil)).Return([]*ent.User{target}, "", false, nil)

	spanExporter := tracetest.NewInMemoryExporter()
	tp := trace.NewTracerProvider(trace.WithSpanProcessor(trace.NewSimpleSpanProcessor(spanExporter)))
	previousProvider := otel.GetTracerProvider()
	otel.SetTracerProvider(tp)
	t.Cleanup(func() {
		_ = tp.Shutdown(context.Background())
	...
	_, _, _, err := service.SearchUsers(ctx, current.ID, model.SearchUserRequest{Query: "target", Limit: 10, IncludeChatID: true})
	require.NoError(t, err)
	...
	require.True(t, names["service.user.repository.search_users"])
	require.True(t, names["service.user.database.private_chats"])
}
