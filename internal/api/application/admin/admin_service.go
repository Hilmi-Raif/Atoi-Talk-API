...
func NewAdminService(client *ent.Client, cfg *config.AppConfig, validator *validator.Validate, wsHub events.Publisher, sessionStore repository.SessionStore, groupMemberRepo repository.GroupMemberReader, storageAdapter objectstorage.URLGenerator) *AdminService {
	return &AdminService{
		client:          client,
		cfg:             cfg,
		validator:       validator,
...
func (s *AdminService) ResolveReport(ctx context.Context, adminID uuid.UUID, reportID uuid.UUID, req model.ResolveReportRequest) error {
	...
	tx, err := s.client.Tx(ctx)
	if err != nil {
		slog.Error("Failed to start report resolution transaction", "error", err)
		return helper.NewInternalServerError("")
	}
	defer func() {
		_ = tx.Rollback()
	...
	update := tx.Report.UpdateOneID(reportID).
	...
	if report.Status(req.Status) == report.StatusResolved {
		if r.TargetType == report.TargetTypeMessage && r.MessageID != nil {
			msgID := *r.MessageID
			...
			if err == nil {
				now := time.Now().UTC()
				shouldPublishDelete := msg.DeletedAt == nil
				updateMsg := tx.Message.UpdateOne(msg).
					ClearContent().
					ClearAttachments()

				if shouldPublishDelete {
					updateMsg.SetDeletedAt(now)
				}

				if err := updateMsg.Exec(ctx); err != nil {
					slog.Error("Failed to delete reported message", "error", err, "messageID", msgID)
					return helper.NewInternalServerError("")
				}
				...
				if shouldPublishDelete {
					if _, err := tx.MessageOutbox.Create().
						...
						SetSenderID(adminID).
						Save(ctx); err != nil {
						slog.Error("Failed to persist reported message delete outbox event", "error", err, "messageID", msgID)
						return helper.NewInternalServerError("")
					}
				}
			}
			if err != nil && !ent.IsNotFound(err) {
				slog.Error("Failed to query reported message", "error", err, "messageID", msgID)
				return helper.NewInternalServerError("")
			}
		}
	}
	...
	if err := tx.Commit(); err != nil {
		slog.Error("Failed to commit report resolution", "error", err, "reportID", reportID)
		return helper.NewInternalServerError("")
	}
...
func (s *AdminService) GetDashboardStats(ctx context.Context) (*model.DashboardStatsResponse, error) {
	var totalUsers, totalGroups, totalMessages, activeReports int
	g, gCtx := errgroup.WithContext(ctx)

	g.Go(func() error {
		count, err := s.client.User.Query().Count(gCtx)
		if err != nil {
	...
	g.Go(func() error {
		count, err := s.client.GroupChat.Query().Count(gCtx)
		if err != nil {
	...
	g.Go(func() error {
		count, err := s.client.Message.Query().Count(gCtx)
		if err != nil {
	...
	g.Go(func() error {
		count, err := s.client.Report.Query().
