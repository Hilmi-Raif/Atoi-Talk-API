...
func TestAdminServiceResolveReportDeletesActiveMessageWhenResolved(t *testing.T) {
	...
	outbox, err := client.MessageOutbox.Query().
		Where(messageoutbox.MessageIDEQ(msg.ID), messageoutbox.EventTypeEQ(string(websocket.EventMessageDelete))).
		Only(ctx)
	require.NoError(t, err)
	require.Equal(t, chatEntity.ID, outbox.ChatID)
	publisher.AssertNotCalled(t, "BroadcastToChat", mock.Anything, mock.Anything)
}

func TestAdminServiceResolveReportPersistsDeleteOutboxWithoutDirectBroadcast(t *testing.T) {
	service, client, _, publisher := newAdminServiceTest(t)
	defer func() { _ = client.Close() }()
	ctx := context.Background()
	reporter := client.User.Create().SetUsername("reporter-outbox").SaveX(ctx)
	sender := client.User.Create().SetUsername("sender-outbox").SaveX(ctx)
	admin := client.User.Create().SetUsername("admin-outbox").SetRole(user.RoleAdmin).SaveX(ctx)
	chatEntity := client.Chat.Create().SetType(chat.TypePrivate).SaveX(ctx)
	client.PrivateChat.Create().SetChat(chatEntity).SetUser1ID(reporter.ID).SetUser2ID(sender.ID).SaveX(ctx)
	msg := client.Message.Create().SetChatID(chatEntity.ID).SetSenderID(sender.ID).SetContent("reported outbox").SaveX(ctx)
	reportEntity := client.Report.Create().
		SetTargetType(report.TargetTypeMessage).
		SetReason("abuse").
		SetReporterID(reporter.ID).
		SetMessageID(msg.ID).
	...
	require.NoError(t, service.ResolveReport(ctx, admin.ID, reportEntity.ID, model.ResolveReportRequest{
