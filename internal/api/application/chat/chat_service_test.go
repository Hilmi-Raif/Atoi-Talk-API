...
func TestChatServiceGetChatsEmitsPhaseSpans(t *testing.T) {
	service, client, chatRepo, _, redisOnline := newChatServiceTest(t)
	defer client.Close()
	...
	redisOnline.EXPECT().ExistsMany(mock.Anything, []string{"online:" + other.ID.String()}).Return(map[string]bool{}, nil)

	_, _, _, err := service.GetChats(ctx, current.ID, model.GetChatsRequest{})
	require.NoError(t, err)
	...
	require.True(t, names["service.chat.repository.get_chats"])
	require.True(t, names["service.chat.redis.presence"])
}
