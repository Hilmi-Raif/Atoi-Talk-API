...
type privateChatCache = redisinfra.Cache
type privateChatStorage = objectstorage.PublicURLGenerator

func NewPrivateChatService(client *ent.Client, cfg *config.AppConfig, validator *validator.Validate, wsHub events.Publisher, redisAdapter privateChatCache, storageAdapter privateChatStorage) *PrivateChatService {
...
func (s *PrivateChatService) CreatePrivateChat(ctx context.Context, userID uuid.UUID, req model.CreatePrivateChatRequest) (*model.ChatResponse, error) {
	...
	if s.wsHub != nil {
		go func() {
			...
			s.wsHub.BroadcastToUser(targetUser.ID, events.Event{
				Type:    events.EventChatNew,
				Payload: payloadForTarget,
				Meta:    &events.EventMeta{Timestamp: time.Now().UTC().UnixMilli(), ChatID: newChat.ID, SenderID: userID},
			})
			...
			s.wsHub.BroadcastToUser(creator.ID, events.Event{
				Type:    events.EventChatNew,
